...
#'
#' Plots multiple line chart with multiple custom x values
#'
#'
#' @param dataset       List of numeric vectors specifying the x-axis values and the corresponding
#'                      data points for the specified xaxis values.Order is very important, see example for details.
...
#' @examples
#' dataset <- list(x1=c(10, 30, 45, 50, 70, 100),
#'                 x2=c(30, 50, 75, 100, 120),
#'                 data1=c(30, 200, 100, 400, 150, 250),
#'                 data2=c(20, 180, 240, 100, 190))
#'
...
#' axis_regions <- list(list(axis='x',start= 30,end= 50,reg_class='regionX'),
#'                      list(axis='x',start= 75,end= 100,reg_class='regionX'))
#'
#'
#'
#' p3_multiple_xy_chart(dataset,colors,show_points=TRUE)
#' p3_multiple_xy_chart (dataset,colors,FALSE,axis_regions,axis_labels,labels_pos,subchart=TRUE)
#' p3_multiple_xy_chart (dataset,colors,FALSE,axis_labels=axis_labels,labels_pos=labels_pos,subchart=TRUE)

#' @import htmlwidgets
#'
#' @export
p3_multiple_xy_chart <- function(dataset,colors=NULL,show_points=FALSE,axis_regions=NULL,
...
#' Shiny bindings for p3_multiple_xy_chart
#'
...
#' @param expr An expression that generates a p3_multiple_xy_chart
#' @param env The environment in which to evaluate \code{expr}.
#' @param quoted Is \code{expr} a quoted expression (with \code{quote()})? This
#'   is useful if you want to save an expression in a variable.
#'
#' @name p3_multiple_xy_chart-shiny
#'
#' @export
p3_multiple_xy_chartOutput <- function(outputId, width = '100%', height = '400px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'p3_multiple_xy_chart', width, height, package = 'PantheraWidgets')
}

#' @rdname p3_multiple_xy_chart-shiny
#' @export
renderP3_multiple_xy_chart <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  htmlwidgets::shinyRenderWidget(expr, p3_multiple_xy_chartOutput, env, quoted = TRUE)
}
