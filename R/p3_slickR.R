...
#' Shiny bindings for p3_slickR
#'
#' Output and render functions for using p3_slickR within Shiny
...
#' @param expr An expression that generates a p3_slickR
#' @param env The environment in which to evaluate \code{expr}.
#' @param quoted Is \code{expr} a quoted expression (with \code{quote()})? This
#'   is useful if you want to save an expression in a variable.
#'
#' @name p3_slickR-shiny
#'
#' @export
...
#' @export
renderP3_slickR <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  htmlwidgets::shinyRenderWidget(expr, p3_slickROutput, env, quoted = TRUE)
}
