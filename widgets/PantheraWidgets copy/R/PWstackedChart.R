...
#' @export
#'
  PWstackedChart <- function(dataset, colors, types = NULL, groups = NULL, subchart = FALSE, axis_type = "timeseries", categories = NULL, width = NULL, height = NULL) {
  ...
  #' Shiny bindings for PWstackedChart
  #'
  #' Output and render functions for using PWstackedChart within Shiny
  ...
  #' @param expr An expression that generates a PWstackedChart
  #' @param env The environment in which to evaluate \code{expr}.
  #' @param quoted Is \code{expr} a quoted expression (with \code{quote()})? This
  #'   is useful if you want to save an expression in a variable.
  #'
  #' @name PWstackedChart-shiny
  #'
  #' @export
  PWstackedChartOutput <- function(outputId, width = '100%', height = '400px'){
    htmlwidgets::shinyWidgetOutput(outputId, 'PWstackedChart', width, height, package = 'PantheraWidgets')
  }

  #' @rdname PWstackedChart-shiny
  #' @export
  renderPWstackedChart <- function(expr, env = parent.frame(), quoted = FALSE) {
    if (!quoted) { expr <- substitute(expr) } # force quoted
    htmlwidgets::shinyRenderWidget(expr, PWstackedChartOutput, env, quoted = TRUE)
  }
